...
const Icon = ({ d, size = 16, stroke = "currentColor", sw = 2 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round">{typeof d === "string" ? <path d={d} /> : d}</svg>
);
const UploadIcon = () => <Icon size={42} sw={1.5} stroke="#2b6cb0" d={<><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></>} />;
const FileIcon = () => <Icon d={<><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></>} />;
const BoltIcon = () => <Icon d={<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>} />;
const PrintIcon = () => <Icon size={15} d={<><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></>} />;
const RefreshIcon = () => <Icon size={14} d={<><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></>} />;
...
.dc-header{padding:24px 5%;border-bottom:3px solid var(--c-primary);position:relative;display:flex;align-items:center;gap:14px}
...
@media(max-width:600px){
  .dc-spec-table{min-width:0}
  .dc-spec-table tr{display:flex;flex-wrap:wrap;padding:6px 0}
  .dc-spec-table td{width:auto!important;padding:4px 12px}
  .dc-spec-label{flex:1 1 55%}
  .dc-spec-value{flex:1 1 45%;text-align:left;padding-right:0}
}
...
@media print{
  .dc-page{box-shadow:none!important;border-radius:0!important;max-width:100%!important}
  .dc-section{break-inside:avoid}
  .dc-data-table thead th,.dc-spec-table tr:nth-child(even),.dc-data-table tbody tr:nth-child(even),.dc-header,.dc-footer{-webkit-print-color-adjust:exact;print-color-adjust:exact}
}
`;
}
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 4096,
          system: `You are a document parsing engine. Extract ALL information from the file and return ONLY a valid JSON object (no markdown fences, no preamble) with this structure:
{
  "title": "Document title or descriptive title",
  "summary": "1-2 sentence overview",
  "sections": [
    { "id": "<unique>", "type": "features"|"specs"|"table"|"text"|"key-values"|"notes"|"images", "heading": "Section title", "content": <by type> }
  ]
}
Content by type:
- "features": {"items":["f1","f2"]}
- "specs": {"rows":[{"label":"L","value":"V"}]}
- "table": {"headers":["H1"],"rows":[["v"]]}
- "text": {"body":"paragraph"}
- "key-values": {"pairs":[{"key":"k","value":"v"}]}
- "notes": {"items":["n1"]}
- "images": {"descriptions":[{"alt":"desc","caption":"cap"}]}
Rules: Extract EVERY detail. For CSVs include ALL rows in a table. Detect the best section type for each group. Preserve numbers/units exactly. Return raw JSON only.${extra}`,
  ...
  const exportHTML = () => {
    ...
    const cv = Object.entries(colors).map(([k, v]) => {
      const map = { primary: "c-primary", accent: "c-accent", accentLight: "c-accent-light", surface: "c-surface", text: "c-text", text2: "c-text2", text3: "c-text3", border: "c-border" };
      return `--${map[k]}:${v}`;
    }).join(";");
...
<title>${sections?.title || "Document"}</title>
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>:root{${cv};font-family:'DM Sans',system-ui,sans-serif;color:var(--c-text)}body{background:var(--c-surface);margin:0;padding:clamp(12px,3vw,32px)}
${getSharedCSS()}
@media print{body{background:#fff!important;padding:0}.no-print{display:none!important}}</style>
  ...
  return (
    <>
      <style>{`
        ...
        .dc-swatch input{opacity:0;position:absolute;inset:0;width:100%;height:100%;cursor:pointer}
        ...
        /* Modals */
        .dc-overlay{position:fixed;inset:0;background:rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center;z-index:999;animation:dc-fi .15s}
        @keyframes dc-fi{from{opacity:0}to{opacity:1}}
        ...
        .dc-modal input[type="text"]{width:100%;padding:8px 12px;border:1px solid var(--c-border);border-radius:6px;font-family:inherit;font-size:14px;color:var(--c-text);outline:none;transition:border-color .15s}
        ...
        @media print{
          ...
          .dc-root{padding:0!important}
          .dc-toolbar,.dc-upload-area,.dc-brand-panel,.no-print,.dc-prompt-section{display:none!important}
        }
      `}</style>
      ...
      <div className="dc-root" style={cssVars}>
        ...
        {!sections && !parsing && (
          <div className="dc-upload-area">
            <div className="dc-upload-zone"
              ...
              onClick={() => document.getElementById("dc-fi").click()}
            >
            ...
            <div className="dc-prompt-section">
              <div className="dc-prompt-label"><BoltIcon /> AI Instructions — How should this be organized?</div>
              <div className="dc-prompt-presets">
                {PRESETS.map((p, i) => (
                  <button key={i} className={`dc-preset${activePreset === i ? " active" : ""}`} onClick={() => pickPreset(i)}>{p.label}</button>
                ))}
              </div>
              <textarea className="dc-prompt-input" placeholder="e.g. 'Focus on pricing data and sort by category' — or pick a preset above…" value={customPrompt} onChange={(e) => { setCustomPrompt(e.target.value); setActivePreset(null); }} />
            </div>

            {file && (
              <div className="dc-file-row">
                <div className="dc-file-pill"><FileIcon /> {file.name}</div>
                <button className="dc-btn dc-btn-primary" onClick={processFile}><BoltIcon /> Parse & Generate</button>
              </div>
            )}
        ...
        {parsing && (
          <div className="dc-progress">
            ...
            <p style={{ color: "var(--c-text2)", fontSize: 14, fontWeight: 500 }}>{progress}</p>
          </div>
        )}
          ...
          <>
            <div className="dc-toolbar no-print">
              <div className="dc-tb-group">
                <button className="dc-btn dc-btn-secondary" onClick={reset}><RefreshIcon /> New File</button>
                <button className="dc-btn dc-btn-secondary" onClick={() => setShowBranding(!showBranding)}><PaletteIcon /> Branding</button>
                <span style={{ fontSize: 12, color: "var(--c-text3)" }}>{file?.name}</span>
              </div>
              <div className="dc-tb-group">
                <button className="dc-btn dc-btn-secondary" onClick={exportHTML}><DownloadIcon /> Download HTML</button>
                <button className="dc-btn dc-btn-primary" onClick={() => window.print()}><PrintIcon /> Print / PDF</button>
              </div>
            </div>
            ...
            {showBranding && (
              <div className="dc-brand-panel no-print">
                <div className="dc-brand-title"><PaletteIcon /> Branding & Colors</div>
                <div className="dc-brand-grid">
                  {[["primary","Primary"],["accent","Accent"],["accentLight","Accent Light"],["surface","Background"],["text","Text"],["text2","Text 2nd"],["border","Borders"]].map(([k,l]) => (
                    <div key={k} className="dc-cf">
                      <label>{l}</label>
                      <div className="dc-cf-row">
                        <div className="dc-swatch" style={{ background: colors[k] }}>
                          <input type="color" value={colors[k]} onChange={(e) => setColors((c) => ({ ...c, [k]: e.target.value }))} />
                        </div>
                        <input className="dc-hex" value={colors[k]} onChange={(e) => setColors((c) => ({ ...c, [k]: e.target.value }))} />
                      </div>
                    </div>
                  ))}
                </div>
                <div className="dc-logo-row">
                  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--c-text2)", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                    <ImgIcon /> Upload Logo
            ...
            <div className="dc-page" ref={pageRef} style={cssVars}>
              <div className="dc-header">
                ...
                <div style={{ flex: 1 }}>
                  <div className="dc-header-company">{header.companyName}</div>
                  <div className="dc-header-tagline">{header.tagline}</div>
                </div>
              ...
              {sections.sections?.map((sec) => (
                <div key={sec.id} className="dc-section">
                  ...
                  {sec.type === "features" && (
                    <div className="dc-features">{sec.content.items?.map((it, i) => <div key={i} className="dc-feature-item">{it}</div>)}</div>
                  )}

                  {sec.type === "specs" && (
                    <div className="dc-spec-wrap"><table className="dc-spec-table"><tbody>
                      {(() => { const r = sec.content.rows || []; const p = []; for (let i = 0; i < r.length; i += 2) p.push([r[i], r[i+1]||null]); return p.map(([l,rt],i) => (
                        <tr key={i}>
                          <td className="dc-spec-label">{l.label}</td><td className="dc-spec-value">{l.value}</td>
                          {rt ? <><td className="dc-spec-label">{rt.label}</td><td className="dc-spec-value">{rt.value}</td></> : <><td/><td/></>}
                        </tr>
                      )); })()}
                    </tbody></table></div>
                  )}
                  ...
                  {sec.type === "table" && (
                    <div className="dc-table-wrap"><table className="dc-data-table">
                      <thead><tr>{sec.content.headers?.map((h,i) => <th key={i}>{h}</th>)}</tr></thead>
                      <tbody>{sec.content.rows?.map((row,ri) => <tr key={ri}>{row.map((c,ci) => <td key={ci}>{c}</td>)}</tr>)}</tbody>
                    </table></div>
                  )}
                  ...
                  {sec.type === "key-values" && (
                    <div className="dc-kv-grid">{sec.content.pairs?.map((p,i) => (
                      <div key={i} className="dc-kv-item"><span className="dc-kv-key">{p.key}</span><span className="dc-kv-val">{p.value}</span></div>
                    ))}</div>
                  )}

                  {sec.type === "notes" && <ul className="dc-notes">{sec.content.items?.map((n,i) => <li key={i}>{n}</li>)}</ul>}

                  {sec.type === "images" && sec.content.descriptions?.map((img,i) => (
                    <div key={i} className="dc-img-placeholder">
                      <ImgIcon /><div style={{ fontWeight: 600, marginTop: 8 }}>{img.alt}</div>
                      {img.caption && <div style={{ fontSize: 12, marginTop: 4 }}>{img.caption}</div>}
                    </div>
                  ))}
                </div>
              ))}
              ...
              <div className="dc-footer">
                <p>{footer.line1}</p><p>{footer.line2}</p>
                <button className="dc-btn dc-btn-ghost no-print" style={{ position: "absolute", top: 8, right: 14 }} onClick={() => setEditingFooter(true)}>✏️ Edit</button>
              </div>
            </div>
          </>
        )}
        ...
        {/* ── Modals ── */}
        {editingHeader && (
          <div className="dc-overlay" onClick={() => setEditingHeader(false)}>
            <div className="dc-modal" onClick={(e) => e.stopPropagation()}>
              <h3>Edit Header</h3>
              <label>Company Name</label><input type="text" value={header.companyName} onChange={(e) => setHeader((h) => ({ ...h, companyName: e.target.value }))} />
              <label>Tagline / Report Type</label><input type="text" value={header.tagline} onChange={(e) => setHeader((h) => ({ ...h, tagline: e.target.value }))} />
              <div className="dc-modal-actions"><button className="dc-btn dc-btn-primary" onClick={() => setEditingHeader(false)}>Done</button></div>
            </div>
          </div>
        )}
        {editingFooter && (
          <div className="dc-overlay" onClick={() => setEditingFooter(false)}>
            <div className="dc-modal" onClick={(e) => e.stopPropagation()}>
              <h3>Edit Footer</h3>
              <label>Line 1</label><input type="text" value={footer.line1} onChange={(e) => setFooter((f) => ({ ...f, line1: e.target.value }))} />
              <label>Line 2</label><input type="text" value={footer.line2} onChange={(e) => setFooter((f) => ({ ...f, line2: e.target.value }))} />
              <div className="dc-modal-actions"><button className="dc-btn dc-btn-primary" onClick={() => setEditingFooter(false)}>Done</button></div>
            </div>
          </div>
        )}
      </div>
